...
for (const relative of expected) {
  ...
  if (!fs.existsSync(file)) {
    console.error(`[Archivo share v8.3.5] Falta archivo: ${relative}`);
    process.exit(1);
  }
}
...
if (fs.existsSync(adminEditor)) {
  const backup = `${adminEditor}.backup-remove-share-v835-${new Date().toISOString().replace(/[:.]/g, '-')}`;
  ...
  console.log('[Archivo share v8.3.5] Share eliminado del admin editor si existía.');
}

console.log('[Archivo share v8.3.5] Share minimalista instalado: solo iconos, sin card.');
