...
const PACK = 'GC_HOME_UNIQUE_8_POPUP_ALIAS_FIX_V1';
...
const oldBest = `      const isValid = (row: any): boolean => {
...
const newBest = `      const isValid = (row: any): boolean => {
        const value = first(row, ['valid', 'Valid', 'isValid', 'is_valid'], true);
        return !(value === false || value === 0 || value === '0' || String(value).toLowerCase() === 'false');
      };
      ...
      const canonicalDriverNameKey = (value: unknown): string => {
        ...
        if (['pdiaz', 'pedrodiaz', 'pedrodiaz'].includes(compact)) return 'pdiaz';
        return key;
      };
...
const oldNormalize = `      const normalize = (value: unknown): string => String(value || '')
        .trim()
        .toLowerCase()
        .normalize('NFD')
        .replace(/[\\u0300-\\u036f]/g,'')
...
const newNormalize = `      const normalize = (value: unknown): string => {
        ...
        // Pedro Díaz aparece en telemetría como PEDRO DÍAZ y en su perfil como PDÍAZ.
...
console.log(`[${PACK}] Se muestran 8 pilotos únicos; cada piloto conserva su mejor vuelta válida o, si no tiene válida, su mejor vuelta disponible.`);
