...
const BACKUP_DIR = path.join(ROOT, '_gc_backups', `phase5a3-1-${new Date().toISOString().replace(/[:.]/g, '-')}`);
const BACKUP = path.join(BACKUP_DIR, 'src/server/index.ts');

function fail(message) {
  console.error(`[GC Phase 5A.3.1] ERROR: ${message}`);
  process.exit(1);
}
...
if (after === before) {
  console.log('[GC Phase 5A.3.1] No había cambios pendientes.');
  process.exit(0);
}
...
console.log('');
console.log('[GC Phase 5A.3.1] Aplicación completada.');
console.log(`[GC Phase 5A.3.1] Backup: ${path.relative(ROOT, BACKUP)}`);
console.log('[GC Phase 5A.3.1] Ejecuta ahora npm run check.');
