...
const BACKUP_DIR = path.join(ROOT, '_gc_backups', `phase5a2-1-${new Date().toISOString().replace(/[:.]/g, '-')}`);
const BACKUP = path.join(BACKUP_DIR, 'src/server/index.ts');

function fail(message) {
  console.error(`[GC Phase 5A.2.1] ERROR: ${message}`);
  process.exit(1);
}
...
const cookieReducerSearch = `    .reduce((cookies, part) => {
...
const cookieReducerReplacement = `    .reduce((cookies, part) => {
      const eqIndex = part.indexOf('=');
      if (eqIndex <= 0) return cookies;
      const key = part.slice(0, eqIndex).trim();
      const value = part.slice(eqIndex + 1).trim();
...
replaceExact(
  cookieReducerSearch,
  cookieReducerReplacement,
  1,
  'acumulador específico de cookies'
);
...
if (after == before) {
  console.log('[GC Phase 5A.2.1] No había cambios pendientes.');
  process.exit(0);
}
...
console.log('');
console.log('[GC Phase 5A.2.1] Aplicación completada.');
console.log(`[GC Phase 5A.2.1] Backup: ${path.relative(ROOT, BACKUP)}`);
console.log('[GC Phase 5A.2.1] Ejecuta ahora npm run check.');
