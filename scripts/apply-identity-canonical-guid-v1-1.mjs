...
const profileFile = path.join(projectRoot, 'src', 'pages', 'pilotos', '[id].astro');
const stamp = new Date().toISOString().replace(/[:.]/g, '-');
const backupRoot = path.join(projectRoot, '_gc_backups', `identity-canonical-guid-v1-1-${stamp}`);

function fail(message) {
  console.error(`[GC IDENTITY GUID V1.1] ERROR: ${message}`);
  process.exit(1);
}
  ...
  <script>
    ...
    (() => {
      ...
      const run = async (): Promise<void> => {
        ...
        const repairLink = (link: HTMLAnchorElement): void => {
          ...
          if (avatar) {
            avatar.src = String(
              canonical.avatarUrl ||
              \`/api/pilot-avatar/\${encodeURIComponent(id)}\`
            );
          }
        };
      ...
      run().catch((error: unknown) => {
        console.warn('[GC identity canonical GUID V1.1] Home repair failed',error);
      });
    })();
  </script>
`;
...
if (!profile.includes('GC_IDENTITY_CANONICAL_GUID_PROFILE_V1')) {
  ...
  if (!profile.includes(oldLoadStart)) {
    fail('No se encontró el inicio de load() esperado en [id].astro.');
  }
  ...
  if (!profile.includes(oldTry)) {
    fail('No se encontró el fetch de perfil esperado en [id].astro.');
  }
...
console.log('[GC IDENTITY GUID V1.1] Corrección aplicada.');
console.log('  - Ruta de perfil corregida: src/pages/pilotos/[id].astro');
console.log('  - Don Bixi: perfil canónico 32.');
console.log('  - ID técnico 3903116451 redirige a 32.');
console.log('  - Jesus Sue mantiene el perfil 9.');
