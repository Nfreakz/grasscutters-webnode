#!/usr/bin/env node
/* GC_HOTLAPS_DATA_CORE_BRIDGE_V1_APPLY
 * Non-destructive bridge for /hotlaps.
 * It appends a final inline script that refreshes the visible leaderboard from /api/gc/*.
 * The legacy /api/hotlaps loader remains as fallback.
 */
...
const START = '/* GC_HOTLAPS_DATA_CORE_BRIDGE_V1_START */';
const END = '/* GC_HOTLAPS_DATA_CORE_BRIDGE_V1_END */';

if (!fs.existsSync(hotlapsPath)) {
  console.error('[GC HOTLAPS CORE] Missing src/pages/hotlaps.astro');
  process.exit(1);
}
  ...
  <script is:inline>
    ...
    (() => {
      ...
      const driverName = (row) => text(pick(row, [
        ...
        'driverDisplayName','driverVisibleName','driverName','playerName','pilotName','name'
      ]), 'Piloto');

      const carName = (row) => text(pick(row, [
        'car.displayName','car.visibleName','car.name','car.uiCarName',
        'carDisplayName','carVisibleName','carName','uiCarName'
      ]), 'Coche');

      const trackName = (row) => text(pick(row, [
        'track.displayName','track.visibleName','track.name','track.uiTrackName',
        'trackDisplayName','trackVisibleName','trackName','uiTrackName'
      ]), 'Circuito');
      ...
      const lapDisplay = (row) => text(pick(row, [
        'lapTimeFormatted','lapTimeText','bestLapTime','bestLap','timeFormatted'
      ]), '') || fmtMs(lapMs(row));
      ...
      const fmtSpeed = (value) => speedValue({ maxSpeedKmh: value }) ? \`\${Math.round(Number(value))} km/h\` : '--';
      ...
      function detectActiveFromSnapshot(snapshotData, rows) {
        ...
        const activeTrack = text(pick(activeCombo, [
          'track.displayName','track.visibleName','track.name','trackName'
        ]), '');
      ...
      function updateMetrics(filtered, rows) {
        ...
        const selectedTrack = els.track?.value === 'all'
          ? 'Todos'
          : (els.track?.options[els.track.selectedIndex]?.textContent || state.activeTrackLabel);
      ...
      function orderLabel() {
        const trackLabel = els.track?.value === state.activeTrackValue
          ? 'combo activo'
          : (els.track?.options[els.track.selectedIndex]?.textContent || 'todos los circuitos').toLowerCase();
        ...
        return \`\${trackLabel} · \${validLabel.toLowerCase()} · \${sortLabel.toLowerCase()} · Data Core\`;
      }
      ...
      function drawCore() {
        ...
        if (els.hint) els.hint.textContent = rows.length
          ? \`\${rows.length} pilotos visibles · \${filtered.length} vueltas filtradas de \${state.cache.length} · Data Core\`
          : 'No hay resultados con esos filtros · Data Core';
        ...
        els.rows.innerHTML = rows.map((row, index) => {
          ...
          const link = id
            ? \`<a class="gc-driver-link" href="/pilotos/\${encodeURIComponent(String(id))}">\${name}</a><span class="gc-leaderboard-sub">Perfil público</span>\`
            : \`<strong>\${name}</strong>\`;
          ...
          \`;
        }).join('') || '<tr><td colspan="10">Sin resultados · Data Core</td></tr>';

        document.documentElement.dataset.gcHotlapsDataCore = 'true';
      }

      async function loadCore() {
        if (els.rows) els.rows.innerHTML = '<tr><td colspan="10">Cargando Data Core...</td></tr>';
        ...
        try {
          ...
          if (els.source) {
            els.source.textContent = 'Core';
            els.source.style.color = '';
          }

          drawCore();

          console.info('[GC /hotlaps Data Core Bridge v1]', {
            loaded: state.cache.length,
            activeTrackValue: state.activeTrackValue,
            activeTrackLabel: state.activeTrackLabel
          });
        } catch (error) {
          console.warn('[GC /hotlaps Data Core Bridge v1] Core unavailable, legacy leaderboard remains active:', error);
          document.documentElement.dataset.gcHotlapsDataCore = 'fallback';
          if (els.source) {
            els.source.textContent = 'Legacy';
            els.source.style.color = '';
          }
        } finally {
...
function replaceOrInsert(source, block) {
  ...
  if (index === -1) {
    console.error('[GC HOTLAPS CORE] Missing </AppLayout> anchor in src/pages/hotlaps.astro');
    process.exit(1);
  }
...
if (next === source) {
  console.log('[GC HOTLAPS CORE] No changes needed.');
  process.exit(0);
}

fs.writeFileSync(hotlapsPath, next, 'utf8');
console.log('[GC HOTLAPS CORE] Hotlaps Data Core bridge applied to src/pages/hotlaps.astro');
console.log('[GC HOTLAPS CORE] Run: npm run build');
