...
if (!fs.existsSync(target)) {
  console.error('[GC APP BEST LOGIC FIX] No encuentro src/pages/app.astro');
  process.exit(1);
}
...
if (content.includes('GC_APP_COMBO_BEST_DRIVER_LOGIC_V15_35_9_1')) {
  console.log('[GC APP BEST LOGIC FIX] v15.35.9.1 ya parece aplicado.');
  process.exit(0);
}

/**
 * 1) Cargar suficientes vueltas para calcular mejor piloto del combo.
 *    No toca imágenes ni assets.
 */
content = content.replace(/\/api\/hotlaps\?limit=12/g, '/api/hotlaps?limit=300');
content = content.replace(/\/api\/hotlaps\?limit=300/g, '/api/hotlaps?limit=300');
content = content.replace(/\/api\/laps\?limit=12&sort=recent&valid=all/g, '/api/laps?limit=500&sort=recent&valid=all');

/**
 * 2) Asegurar lapMs robusto. Si ya lo es, no lo tocamos.
 */
const simpleLapMs = `      const lapMs = (lap) => number(pick(lap, ['lapTimeMs','LapTime','lapTime','timeMs','time']), Number.POSITIVE_INFINITY);`;
const robustLapMs = `      const lapMs = (lap) => {
...
if (content.includes(simpleLapMs)) {
  content = content.replace(simpleLapMs, robustLapMs);
}

/**
 * 3) Helper único, robusto, sin depender de variables intermedias viejas.
 */
const helper = `      /* GC_APP_COMBO_BEST_DRIVER_LOGIC_V15_35_9_1 START */
      ...
      function gcBestSelectComboDriver(combosData, recentData, bestData){
        ...
        const pool = [...recent, ...best].filter((row) => lapMs(row) !== Number.POSITIVE_INFINITY);
      ...
      function gcBestRenderComboDriver(combosData, recentData, bestData){
        ...
        if (typeof renderTopDriver === 'function') {
          renderTopDriver(row);
        }
        ...
        if (row) {
          ...
          console.warn('[GC /app] No se pudo calcular mejor piloto combo', {
            combo: gcBestActiveCombo(combosData),
      ...
      /* GC_APP_COMBO_BEST_DRIVER_LOGIC_V15_35_9_1 END */

`;

/**
 * 4) Reemplazar helpers viejos si existen.
 */
const oldStart = content.indexOf('function comboNormalize(value)');
const oldEnd = oldStart !== -1 ? content.indexOf('function driverAvatarUrl(row)', oldStart) : -1;

if (oldStart !== -1 && oldEnd !== -1) {
  content = content.slice(0, oldStart) + helper + content.slice(oldEnd);
} else if (!content.includes('function gcBestRenderComboDriver(combosData, recentData, bestData)')) {
  const renderTopIndex = content.indexOf('function renderTopDriver(row)');
  if (renderTopIndex !== -1) {
    content = content.slice(0, renderTopIndex) + helper + content.slice(renderTopIndex);
  } else {
    const renderLapMatch = content.match(/(\n\s*)function\s+renderLapReadout\s*\(/);
    if (!renderLapMatch || renderLapMatch.index === undefined) {
      console.error('[GC APP BEST LOGIC FIX] No encuentro punto para insertar helper.');
      process.exit(1);
    }
    content = content.slice(0, renderLapMatch.index + renderLapMatch[1].length) + helper + content.slice(renderLapMatch.index + renderLapMatch[1].length);
  }
}

/**
 * 5) Si no existe renderTopDriver, crearlo.
 */
...
`;
  const renderLapMatch = content.match(/(\n\s*)function\s+renderLapReadout\s*\(/);
  if (!renderLapMatch || renderLapMatch.index === undefined) {
    console.error('[GC APP BEST LOGIC FIX] No encuentro renderLapReadout para insertar renderTopDriver.');
    process.exit(1);
  }
  content = content.slice(0, renderLapMatch.index + renderLapMatch[1].length) + avatarHelper + content.slice(renderLapMatch.index + renderLapMatch[1].length);
}

/**
 * 6) Quitar llamadas viejas que podían dejar la card vacía.
 */
...
/**
 * 7) Insertar llamada final de forma robusta.
 *    Anclaje estable: justo antes de setText('gcAppStatusChip','Live')
 */
if (!content.includes('gcBestRenderComboDriver(combosData, recentLaps, bestHotlaps);')) {
  const liveNeedleSingle = "setText('gcAppStatusChip','Live');";
  const liveNeedleSpaced = "setText('gcAppStatusChip', 'Live');";

  if (content.includes(liveNeedleSingle)) {
    content = content.replace(liveNeedleSingle, "gcBestRenderComboDriver(combosData, recentLaps, bestHotlaps);\n        " + liveNeedleSingle);
  } else if (content.includes(liveNeedleSpaced)) {
    content = content.replace(liveNeedleSpaced, "gcBestRenderComboDriver(combosData, recentLaps, bestHotlaps);\n        " + liveNeedleSpaced);
  } else {
    // fallback: después de renderLapReadout(...)
    const renderLapCall = content.match(/renderLapReadout\([^\n;]*\);\s*/);
    if (!renderLapCall || renderLapCall.index === undefined) {
      console.error('[GC APP BEST LOGIC FIX] No encuentro dónde insertar la llamada final.');
      process.exit(1);
    }
    const insertAt = renderLapCall.index + renderLapCall[0].length;
    content = content.slice(0, insertAt) + "        gcBestRenderComboDriver(combosData, recentLaps, bestHotlaps);\n" + content.slice(insertAt);
  }
}

/**
 * 8) Marcador CSS mínimo, sin tocar imágenes.
 */
const css = `
    /* GC_APP_COMBO_BEST_DRIVER_LOGIC_V15_35_9_1_STYLE */
...
if (!content.includes('GC_APP_COMBO_BEST_DRIVER_LOGIC_V15_35_9_1_STYLE')) {
  const close = '  </style>';
  if (!content.includes(close)) {
    console.error('[GC APP BEST LOGIC FIX] No encuentro cierre </style>.');
    process.exit(1);
  }
  content = content.replace(close, css + '\n' + close);
}

const backup = target + '.bak-v15-35-9-1-combo-best-driver-logic';
if (!fs.existsSync(backup)) fs.writeFileSync(backup, original, 'utf8');

fs.writeFileSync(target, content, 'utf8');

console.log('[GC APP BEST LOGIC FIX] v15.35.9.1 aplicado.');
console.log('[GC APP BEST LOGIC FIX] Backup: ' + backup);
console.log('[GC APP BEST LOGIC FIX] Ejecuta: npm run build');
