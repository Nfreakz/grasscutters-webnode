...
const BACKUP_DIR = path.join(ROOT, '_gc_backups', `phase5a4-${new Date().toISOString().replace(/[:.]/g, '-')}`);
const BACKUP = path.join(BACKUP_DIR, 'src/server/index.ts');

function fail(message) {
  console.error(`[GC Phase 5A.4] ERROR: ${message}`);
  process.exit(1);
}
...
replaceExact(
  'compactLapForCombo({ ...lap, comboId: rawComboId ?? lap.comboId, comboKey: key, comboUid: key })',
  'compactLapForCombo({ ...lap, comboId: rawComboId ?? lap.comboId, comboKey: key, comboUid: key } as any)',
  2,
  'payloads comboKey'
);

replaceExact(
  'context.user.team.role = normalizeGcTeamRole(teamRole)',
  'context.user.team!.role = normalizeGcTeamRole(teamRole)',
  1,
  'team ya validado'
);

const duplicateOkSearch = `      res.status(200).json({
        ok: false,
        authenticated: true,
...
const duplicateOkReplacement = `      res.status(200).json({
        ...gcPublicDataCoreUnavailableV130(readSource, 'Data Core no disponible para generar el perfil.'),
        ok: false,
        authenticated: true,
        user: publicUser(context.user),
...
replaceExact(
  duplicateOkSearch,
  duplicateOkReplacement,
  1,
  'orden de spread y ok'
);

if (after === before) {
  console.log('[GC Phase 5A.4] No había cambios pendientes.');
  process.exit(0);
}
...
console.log('');
console.log('[GC Phase 5A.4] Aplicación completada.');
console.log(`[GC Phase 5A.4] Backup: ${path.relative(ROOT, BACKUP)}`);
console.log('[GC Phase 5A.4] Ejecuta ahora npm run check.');
