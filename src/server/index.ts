...
/* GC_COMBOS_LOGICAL_UNIFY_V1_START */
type GcLogicalComboLapAny = any;

function gcComboUnifySlugV1(value: any) {
  return String(value ?? '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
...
function gcComboUnifyFirstTextV1(...values: any[]) {
  for (const value of values) {
    const text = compactNullableText(value);
    if (text) return text;
  }
  return null;
}

function gcComboUnifySourceKeyV1(lap: any) {
  return gcComboUnifyFirstTextV1(lap?.sourceKey, lap?.source_key, lap?.source) || 'main';
}

function gcComboUnifySourceLabelV1(sourceKey: string, lap: any) {
...
function gcComboUnifyRawComboKeyV1(lap: any) {
  ...
  const rawCombo = gcComboUnifyFirstTextV1((lap as any)?.rawComboId, lap?.comboId, lap?.combo?.id);
  if (rawCombo) return `${sourceKey}:${rawCombo}`;
  const legacy = getLegacyComboKeyFromLap(lap as ComboLap);
  return legacy ? `${sourceKey}:${legacy}` : null;
}

function gcComboUnifyBuildVilaRealSplitMapV1(laps: any[]) {
  const byScope = new Map<string, Map<string, any>>();

  for (const lap of laps || []) {
    const track = gcComboUnifyTrackFamilyV1(lap?.track);
    if (track.key !== 'vila_real') continue;
    const rawKey = gcComboUnifyRawComboKeyV1(lap);
    if (!rawKey) continue;
    const sourceKey = gcComboUnifySourceKeyV1(lap);
    const scope = `${sourceKey}:vila_real`;
    if (!byScope.has(scope)) byScope.set(scope, new Map());
    const rawMap = byScope.get(scope)!;
    if (!rawMap.has(rawKey)) rawMap.set(rawKey, { rawKey, minTs: Number.POSITIVE_INFINITY, maxTs: 0, laps: 0 });
    const bucket = rawMap.get(rawKey);
    const ts = Number(lap?.timestamp ?? 0);
    if (Number.isFinite(ts) && ts > 0) {
      bucket.minTs = Math.min(bucket.minTs, ts);
      bucket.maxTs = Math.max(bucket.maxTs, ts);
    }
    bucket.laps += 1;
  }

  const result = new Map<string, string>();
  const minSplitGapSeconds = 18 * 60 * 60;

  for (const rawMap of byScope.values()) {
    const clusters = Array.from(rawMap.values())
      .filter((item) => Number.isFinite(item.minTs) && item.maxTs > 0)
    ...
    for (let i = 0; i < clusters.length - 1; i += 1) {
      const gap = Math.max(0, Number(clusters[i + 1].minTs) - Number(clusters[i].maxTs));
      if (gap > bestGap) {
        bestGap = gap;
        bestIndex = i;
      }
    ...
    if (bestIndex >= 0 && bestGap >= minSplitGapSeconds) {
      clusters.forEach((cluster, index) => result.set(cluster.rawKey, index <= bestIndex ? 'pre_chicane' : 'post_chicane'));
    }
  }
...
function gcComboUnifyTrackVariantV1(lap: any, vilaSplitMap: Map<string, string>) {
  const track = gcComboUnifyTrackFamilyV1(lap?.track);
  const slug = track.slug || '';
  if (track.key !== 'vila_real') return 'default';
  if (/pre|old|no_chicane|nochicane|sin_chicane/.test(slug)) return 'pre_chicane';
  if (/post|chicane|new|2026/.test(slug)) return 'post_chicane';
  const rawKey = gcComboUnifyRawComboKeyV1(lap);
  return (rawKey && vilaSplitMap.get(rawKey)) || 'default';
}

function gcComboUnifyVariantLabelV1(trackKey: string, variant: string) {
  if (trackKey === 'vila_real' && variant === 'pre_chicane') return 'Pre chicane';
  if (trackKey === 'vila_real' && variant === 'post_chicane') return 'Post chicane';
  return null;
}

function gcComboUnifyCarMapKeyV1(car: any) {
  const tokens = gcComboUnifyCarTokensV1(car);
...
function gcComboUnifyCarDisplayV2(lap: any) {
  const raw = gcComboUnifyCarTokensV1(lap?.car);
  const family = gcComboUnifyCarFamilyV1(lap?.car);
  return {
    ...(lap?.car || {}),
...
function gcComboUnifyBuildStatsV1(allLaps: ComboLap[]) {
  const vilaSplitMap = gcComboUnifyBuildVilaRealSplitMapV1(allLaps as any[]);
  const map = new Map<string, any>();
  ...
  for (const lap of allLaps || []) {
    ...
    if (!map.has(key)) {
      ...
      map.set(key, {
        ...
        comboKey: key,
        comboId: null,
        canonicalComboId: null,
        memberComboIds: [],
        mergedCombosCount: 0,
        mergePolicy: gcComboUnifyMakePublicPolicyV2(),
        url: `/combos/${encodeURIComponent(key)}`,
        detailUrl: `/combos/${encodeURIComponent(key)}`,
        filterUrl: `/combos?source=${encodeURIComponent(sourceKey)}&track=${encodeURIComponent(trackFamily.key)}&variant=${encodeURIComponent(variant)}`,
        sourceKey,
        sourceLabel,
        trackId: lap.track?.id ?? null,
        carId: null,
        ...
        cars: [],
        carIds: [],
        carGroupKey: 'public_car_pool',
        carGroupName: 'Coches públicos del combo',
        carSummary: 'Coches del combo',
        usedCars: [],
        usedCarSummary: 'Coches del combo',
        totalLaps: 0,
        validLaps: 0,
        invalidLaps: 0,
        drivers: new Map(),
    ...
    const entry = map.get(key);
    const rawComboKey = gcComboUnifyRawComboKeyV1(lap);
    if (rawComboKey) entry.rawCombos.add(rawComboKey);
    const rawComboId = getComboIdFromLap(lap as ComboLap);
    if (rawComboId !== null) entry.memberComboIds = Array.from(new Set([...(entry.memberComboIds || []), rawComboId])).sort((a: number, b: number) => a - b);
    entry.mergedCombosCount = Math.max(entry.rawCombos.size, entry.memberComboIds.length, 1);

    entry.totalLaps += 1;
    lap.valid ? entry.validLaps += 1 : entry.invalidLaps += 1;
    entry.totalCuts += Math.max(0, Number(lap.cuts) || 0);

    const driverKey = gcComboUnifyDriverKeyV2(sourceKey, lap);
    if (driverKey) entry.drivers.set(driverKey, lap.driver || { name: compactNullableText(lap?.driverName || lap?.playerName) || 'Piloto' });
    if (lap.sessionId !== null && lap.sessionId !== undefined) entry.sessions.add(`${sourceKey}:${lap.sessionId}`);

    const carKey = gcComboUnifyCarBucketKeyV2(lap);
    const carDisplay = gcComboUnifyCarDisplayV2(lap);
    if (!entry.carStatsMap.has(carKey)) {
      entry.carStatsMap.set(carKey, {
        key: carKey,
        car: carDisplay,
        totalLaps: 0,
        validLaps: 0,
    ...
    const carStats = entry.carStatsMap.get(carKey);
    carStats.totalLaps += 1;
    if (lap.valid) carStats.validLaps += 1;
    carStats.drivers.add(driverKey);
    if (Number(lap.timestamp || 0) > Number(carStats.lastSeenTimestamp || 0)) carStats.lastSeenTimestamp = Number(lap.timestamp || 0);
    entry.usedCarsMap.set(carKey, carDisplay);
  ...
  return Array.from(map.values())
    .map((entry) => {
      ...
      entry.hiddenLowLapCars = hiddenLowLapCars.map((item: any) => ({
        key: item.key,
        name: item.car?.name || item.car?.displayName || item.key,
        totalLaps: item.totalLaps,
        validLaps: item.validLaps,
        driversCount: item.drivers?.size || 0
      }));
      entry.usedCars = publicCars;
      entry.cars = publicCars;
      entry.carIds = publicCars.map((car: any) => car?.id).filter((id: any) => id !== null && id !== undefined);
      entry.carSummary = carSummaryFromCars(publicCars.length ? publicCars : [{ name: entry.carGroupName }]);
      entry.usedCarSummary = entry.carSummary;
      entry.allCarsCount = carStats.length;
      entry.hiddenLowLapCarsCount = hiddenLowLapCars.length;
      entry.publicCarsCount = publicCars.length;
      entry.driversCount = entry.drivers.size;
      entry.sessionsCount = entry.sessions.size;
      return entry;
    })
    .filter((entry) => {
    ...
    .map((item: any) => {
      const stableComboId = item.comboUid || item.comboKey || item.key || item.canonicalComboId || item.comboId;
      return {
      ...item,
      comboUid: item.comboUid || item.key,
      comboKey: item.comboKey || item.key,
      comboId: item.comboId ?? stableComboId,
      canonicalComboId: item.canonicalComboId ?? item.comboId ?? stableComboId,
      sourceKey: item.sourceKey || (String(item.key || '').split(':')[0] || 'main'),
      sourceLabel: item.sourceLabel || (String(item.key || '').startsWith('gt4:') ? 'Supra GT4' : 'Liga GrassCutters'),
      carGroupKey: item.carGroupKey || null,
      carGroupName: item.carGroupName || item.carSummary,
      rawCombosCount: item.mergedCombosCount || 1,
      publicPolicy: item.publicPolicy || item.mergePolicy || gcComboUnifyMakePublicPolicyV2(),
      totalCarsCount: item.allCarsCount || item.carsCount || (item.cars || []).length,
      hiddenLowLapCarsCount: item.hiddenLowLapCarsCount || 0
      };
    })
    .sort((a, b) => Number(b.lastSeenTimestamp ?? 0) - Number(a.lastSeenTimestamp ?? 0));
}

function gcComboUnifyMatchesFilterV1(combo: any, filterValue: string | null) {
  if (!filterValue) return true;
  const wanted = gcComboUnifySlugV1(filterValue);
  if (!wanted) return true;
  const values = [
    combo?.track?.code,
    combo?.track?.familyKey,
    combo?.track?.name,
    combo?.track?.rawCode,
    combo?.track?.rawName,
    combo?.carGroupKey,
    ...
    ...(combo?.cars || []).flatMap((car: any) => [car?.code, car?.name, car?.brand])
  ].map(gcComboUnifySlugV1);
  return values.some((value) => value === wanted || value.includes(wanted) || wanted.includes(value));
}
/* GC_COMBOS_LOGICAL_UNIFY_V1_END */
...
app.get('/api/gc/combos', async (req: any, res: any) => {
  try {
    ...
    const dataCoreSource = readSource as GcDataCoreReadResult;
    let items = gcComboUnifyBuildStatsV1(dataCoreSource.laps);
    ...
    const trackFilter = getQueryString(req, 'track') || getQueryString(req, 'trackFamily');
    const carFilter = getQueryString(req, 'carGroup') || getQueryString(req, 'car');
    const variantFilter = getQueryString(req, 'variant');
    if (trackFilter) items = items.filter((combo: any) => gcComboUnifyMatchesFilterV1(combo, trackFilter));
    if (carFilter) items = items.filter((combo: any) => gcComboUnifyMatchesFilterV1(combo, carFilter));
    if (variantFilter) {
      const wantedVariant = gcComboUnifySlugV1(variantFilter);
      items = items.filter((combo: any) => gcComboUnifySlugV1(combo?.track?.variant || '') === wantedVariant);
    }
    ...
    res.json({
      ...
      policy: {
        ...
        identity: 'sourceKey + normalizedTrackFamily + trackVariant'
      },
      message: dataCoreSource.source === 'mysql-mirror-v2'
        ? 'Combos públicos generados desde Mirror V2 combinado con agrupación lógica por circuito.'
        : 'Combos públicos generados desde GC Data Core con agrupación lógica por circuito.'
    });
  } catch (error) {
...
function gcComboDecodeRequestIdV14(value: any) {
  const raw = String(value ?? '').trim();
  if (!raw) return '';
  try { return decodeURIComponent(raw); } catch { return raw; }
}

function gcComboComparableIdV14(value: any) {
  const decoded = gcComboDecodeRequestIdV14(value);
  return decoded ? decoded.toLowerCase() : '';
}

function gcComboSourceAwareIdCandidatesV14(combo: any) {
  const values = [
    combo?.comboUid,
    combo?.comboKey,
    combo?.key,
    combo?.canonicalComboId,
    combo?.comboId,
    combo?.id,
    combo?.detailUrl,
    combo?.url,
  ...
  return values
    ...
    .flatMap((value) => {
      const text = String(value).trim();
      const tail = text.includes('/') ? text.split('/').filter(Boolean).pop() || text : text;
      return [text, tail];
    })
    .map(gcComboComparableIdV14)
    .filter(Boolean);
}

function gcComboSourceAwareFindByRequestV14(items: any[], requestedId: string) {
  const wanted = gcComboComparableIdV14(requestedId);
...
app.get('/api/gc/combos/:comboId', async (req: any, res: any) => {
  try {
    ...
    const dataCoreSource = readSource as GcDataCoreReadResult;
    const sourceAwareCombos = gcComboUnifyBuildStatsV1(dataCoreSource.laps);
    let combo = gcComboSourceAwareFindByRequestV14(sourceAwareCombos, requestedId);
    let comboCore = 'gc-combo-public-logical-v2';

    // Compatibilidad: permite abrir fichas antiguas por ID numérico/raw si algún enlace viejo quedó cacheado.
    if (!combo) {
      const legacyCombos = buildComboStatsFromLaps(dataCoreSource.laps, dataCoreSource.comboDefinitions || []);
      combo = legacyCombos.find((entry: any) => gcComboDetailComboMatchesIdV1(entry, requestedId));
      comboCore = 'gc-combo-legacy-compat-v1';
    }
    ...
    if (!combo) {
      return res.status(404).json({
        ...
        comboId: requestedId,
        availableHint: sourceAwareCombos.slice(0, 20).map((item: any) => item.comboUid || item.comboKey || item.key || item.comboId)
      });
    }

    const rows = Array.isArray(combo.laps) && combo.laps.length
      ? combo.laps
      : dataCoreSource.laps.filter((lap: any) => gcComboDetailLapMatchesComboV1(lap, combo));
    const item = gcComboDetailBuildItemV1(combo, rows);
    const stableComboId = combo.comboUid || combo.comboKey || combo.key || item.comboId || requestedId;
    ...
    res.json({
      ...
      item: {
        ...item,
        comboId: stableComboId,
        canonicalComboId: combo.canonicalComboId || stableComboId,
        comboUid: combo.comboUid || stableComboId,
        comboKey: combo.comboKey || stableComboId,
        sourceKey: combo.sourceKey || item.sourceKey || (String(stableComboId).startsWith('gt4:') ? 'gt4' : 'main'),
        sourceLabel: combo.sourceLabel || item.sourceLabel || (String(stableComboId).startsWith('gt4:') ? 'Supra GT4' : 'Liga GrassCutters'),
        detailUrl: combo.detailUrl || `/combos/${encodeURIComponent(stableComboId)}`,
        filterUrl: combo.filterUrl || null,
        memberComboIds: Array.isArray(combo.memberComboIds) ? combo.memberComboIds : item.memberComboIds,
        mergePolicy: combo.mergePolicy || combo.publicPolicy || item.mergePolicy || null,
        hiddenLowLapCars: combo.hiddenLowLapCars || [],
        hiddenLowLapCarsCount: combo.hiddenLowLapCarsCount || 0
      },
      meta: {
        requestedComboId: requestedId,
        matchedComboId: stableComboId,
      ...
      message: dataCoreSource.source === 'mysql-mirror-v2'
        ? 'Ficha de combo generada desde Mirror V2 combinado con agrupación lógica pública.'
